// Molecular dynamics codes for simulation case 7: Wt.3D.SPCFw.NVT
...
Sim.Init = function (_N, _Box_Len, _Setpoint_Temperature)
{
	...
	this.Berendsen_Time_Const = 10.0;
	...
	var V_p = Math.sqrt(2.0 * kB * T0 / (PP_Table.SPCFw_O.Mass * amu)) * 1.0e-6;
	
	var i = 0;
	
	var col = 0, row = 0, depth = 0;
	var N_in_row = Math.ceil(Math.pow(this.N_Mol, 1.0 / 3.0));
	var N_in_plane = N_in_row * N_in_row;
	var sx, sy, sz;
	
	for (i = 0; i < this.N_Mol; i ++)
	{
		...
		this.Mol[i] = new Molecule ();
		Model_Builder.AddWater_SPCFw (this, i, sx, sy, sz);
		this.Mol[i].SetRandomLinearVelocity(this, V_p);
	}
