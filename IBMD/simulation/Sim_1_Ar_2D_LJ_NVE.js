// Molecular dynamics codes for simulation case 1: Ar.2D.LJ.NVE
...
Sim.SetAvailablePlots = function (Plot_Manager)
{
	...
	Plot_Manager.Default_Plot = 'Radial Distribution Function';
};
...
Sim.Force = function (i, j)
{
	...
	var Ar_Sig6 = this.Atom[i].Sig6;
	var Ar_Sig12 = this.Atom[i].Sig12;
...
Sim.ChangeTemperature = function (factor)
{
	if (!this.Busy && this.Temperature < 800.0 && this.Started)
	{
		this.Lock();
		
		var i, j;
		
		for (i = 0; i < Math.floor(0.3 * this.N); i ++)
		{
			j = Math.floor(Random(0, this.N - 1));
			
			this.Atom[j].vx *= factor;
			this.Atom[j].vy *= factor;
		}
...
Sim.ChangeVolume = function (factor)
{
	if (!this.Busy && ((factor > 1.0 && this.Box_Len < 50.0) || (factor < 1.0 && this.Box_Len > 10.0)) && this.Started)
	{
		this.Lock();
		...
		for (i = 0; i < this.N; i ++)
		{
			this.Atom[i].x *= factor;
			this.Atom[i].y *= factor;
		}

		this.Volume = this.Box_Len * this.Box_Len * this.Thickness;
